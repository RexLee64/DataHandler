/*
 * @Author: Rex 
 * @Date: 2018-09-25 10:20:51 
 * @Last Modified by:  
 * @Last Modified time: 2018-09-26 16:39:50
 * @description: PF_PACKET SOCK_DGARM数据包解析demo
 */
...
#define BUFF_SIZE 2048                       // 接收缓冲区大小
static const char *g_szIfName = "ens3f1np1"; // 物理网卡接口,需要根据具体情况修改
...
void analyseUDP(UDP_HEADER *udp, IP_HEADER *ip);
...
int main(int argc, char *argv[])
{
    ...
    strncpy(ifr.ifr_name, g_szIfName, IFNAMSIZ); // 指定网卡名称
    if (-1 == ioctl(sockfd, SIOCGIFINDEX, &ifr)) // 获取网络接口
    {
        perror("ioctl");
        close(sockfd);
        return -1;
    ...
    // if (ioctl(sockfd, SIOCSIFFLAGS, &ifr) == -1) // 设置接口标志
    // {
    //     perror("ioctl");
    //     close(sockfd);
    //     return -1;
    ...
    while (1)
    {
        ...
        // 接收数据不包括数据链路帧头.
        // Ethernet (14), IP (20) and TCP/UDP(8)

        ip = (IP_HEADER *)(buf);
        // analyseIP(ip);
        // size_t iplen = (ip->h_verlen & 0x0f) * 4;
        size_t iplen = sizeof(IP_HEADER);
    ...
    close(sockfd);
    ifr.ifr_flags &= ~IFF_PROMISC;               // 取消网卡混杂模式
    if (ioctl(sockfd, SIOCSIFFLAGS, &ifr) == -1) // 设置接口标志
    {
        perror("ioctl");
        close(sockfd);
