/*
 * @Author: csdn-writer
 * @Date: 2018-09-25 10:49:18 
 * @Last Modified by:  
 * @Last Modified time: 2018-09-25 10:58:02
 * @description: libpcap demo
 */
...
int main(int argc, charchar *argv[])
{
    ...
    const unsigned charchar *p_packet_content = NULL; // 保存接收到的数据包的起始地址
    unsigned charchar *p_mac_string = NULL;           // 保存mac的地址，临时变量
    unsigned short ethernet_type = 0;                 // 以太网类型
    charchar *p_net_interface_name = NULL;            // 接口名字
    ...
    printf("Capture Time is :%s", ctime((const time_ttime_t *)&protocol_header.ts.tv_sec));
    ...
    p_mac_string = (unsigned charchar *)ethernet_protocol->ether_shost; //获取源mac
    printf("Mac Source Address is %02x:%02x:%02x:%02x:%02x:%02x\n", *(p_mac_string + 0), *(p_mac_string + 1), *(p_mac_string + 2), *(p_mac_string + 3), *(p_mac_string + 4), *(p_mac_string + 5));
    p_mac_string = (unsigned charchar *)ethernet_protocol->ether_dhost; //获取目的mac
